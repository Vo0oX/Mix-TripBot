...
module.exports = {
    sendTask: async function (row, task_id, callback) {
        ...
        if (row[0].request_type === 'subscribe') {
            ...
            if (!row[0].post_old && !row[0].post_new && row[0].login && !row[0].link) {
                ...
                likemania = `https://likemania.com/api2/task/?key=${process.env.Likemania_TOKEN}&service_id=${row[0].service_id}&amount=${row[0].amount}&url=http://instagram.com/${row[0].login}&force=1`;
